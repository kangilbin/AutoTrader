...
Qty = min(
    ...
    CUR_AMOUNT × MAX_LOSS_PCT / (현재가 - 손절가)  ← 안전장치: 손절 시 배정금의 10% 이내 손실
)
손절가 = 진입 시점 EMA20 - ATR × 1.0
```

- **ENTRY_PCT**: 매 사이클 배정금 대비 투입 비율 (기본 50%)
- **MAX_LOSS_PCT**: 손절 시 배정금 대비 최대 손실률 (기본 10%, 고변동성 안전장치)

보통 주식(ATR 2~3%)에서는 ENTRY_PCT가 먼저 걸려 배정금의 절반을 투입합니다.
급등락 종목(ATR 10%+)에서만 MAX_LOSS_PCT가 발동하여 수량을 추가로 줄입니다.
...
- 현재가 ≤ 고점(PEAK_PRICE) - ATR×2.0 **AND** OBV z-score < -0.5
- 수급(OBV)이 살아있으면 ATR 조건 충족해도 **매도 안 함** → 대시세 추적
- OBV 꺾이면 잔량 전량 매도
