...
SIGNAL 1 (보유 - 1차 익절 전) ← PEAK_PRICE = 매수가
  ...
  ├─ [손절] 매수가-ATR×2.0 (7% cap) ──────→ SIGNAL 3
  └─ [1차 익절] PEAK-ATR×2.0 → 50% 매도 → SIGNAL 2
                                              │ PEAK_PRICE 리셋 (현재가)
                                              │ 장중 5분마다: PEAK_PRICE = max(PEAK_PRICE, 현재가)
                                              ├─ [손절] Max(매수가-ATR×2.0, 평단가) → SIGNAL 3
                                              └─ [2차 익절] PEAK-ATR×2.0 AND OBV-Z14<-0.5 → SIGNAL 3
                                                  (OBV 양호 시 → 추세 추적 계속)

SIGNAL 3 (수급 이탈 대기) — 매도 직후 무조건 진입
  │ OBV z-score < COOLDOWN_OBV_EXIT 확인 (수급 이탈 완료)
  ▼
SIGNAL 4 (수급 재유입 대기)
  │ OBV z-score > COOLDOWN_OBV_REENTRY 확인 (강한 재유입 확인)
  ▼
SIGNAL 0 (매수 대기)
```
...
- OBV 꺾이면 잔량 전량 매도 → SIGNAL 3 (수급 안정화 대기)
...
| **손절** | 매수가 - ATR×2.0 (entity ENTRY_PRICE 기반) | 동일 (매수 시 확정, 일봉 한계) |
| **손절 후 SIGNAL** | 항상 SIGNAL 3 | 동일 |
...
| 3 | **갭 하락 필터** | `현재가 < 전일 저가` | `당일 시가 < 전일 저가` | 실전은 매 체크 시점 현재가가 전일 저가 아래인지 확인 (갭+장중 하락 모두 차단). 백테스팅은 시가만 알 수 있어 갭 하락만 감지 |
| 4 | **1차 익절 수익 체크** | 없음 | 체결가 < 매수가면 익절 스킵 | 실전은 실시간 가격으로 체크하므로 갭하락과 익절이 동시에 걸리지 않음. 백테스팅은 일봉에서 갭하락 시 익절 체결가가 손실가가 될 수 있어 보정 필요 |
