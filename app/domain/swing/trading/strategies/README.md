...
SIGNAL 1 (보유 - 1차 익절 전) ← PEAK_PRICE = 매수가
  ...
  ├─ [손절] EMA-ATR ──────────→ OBV z ≤ 0? → SIGNAL 0
  │                              OBV z > 0? → SIGNAL 3
  └─ [1차 익절] PEAK-ATR×2.0 → 50% 매도 → SIGNAL 2
                                              │ PEAK_PRICE 리셋 (현재가)
                                              │ 장중 5분마다: PEAK_PRICE = max(PEAK_PRICE, 현재가)
                                              ├─ [손절] Max(EMA-ATR, 평단가) → OBV z 분기 → SIGNAL 0 or 3
...
매 사이클마다 배정금의 일정 비율만 투입하고, 고변동성 종목은 손실 한도로 추가 제한합니다.

```
Qty = min(
    CUR_AMOUNT × ENTRY_PCT / 현재가,              ← 기본: 배정금의 50% 투입
    CUR_AMOUNT × MAX_LOSS_PCT / (현재가 - 손절가)  ← 리스크 제한: 손절 시 배정금의 3% 이내 손실
)
손절가 = 진입 시점 EMA20 - ATR × 1.0
```

- **ENTRY_PCT**: 매 사이클 배정금 대비 투입 비율 (기본 50%)
- **MAX_LOSS_PCT**: 손절 시 배정금 대비 최대 손실률 (기본 3%, 리스크 기반 수량 조절)

보통 주식(ATR 2~3%)에서는 ENTRY_PCT가 먼저 걸려 배정금의 절반을 투입합니다.
중변동성 종목(ATR 5%+)부터 MAX_LOSS_PCT가 발동하여 투입 비중을 축소합니다.
...
- **SIGNAL 1**: 현재가 ≤ EMA20 - ATR×1.0 시 전량 매도
- **SIGNAL 2**: 현재가 ≤ Max(EMA20 - ATR×1.0, 평단가) 시 전량 매도 (**본전 방어**)
