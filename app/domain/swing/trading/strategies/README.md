...
아래 **5가지 조건 모두** 충족 후 **연속 2회(약 10분)** 확인 시 진입 (현재가 기준)
...
> 전전일 데이터(`prev_close`, `prev_obv_z`)는 Redis 캐시에 포함되어 실시간 조건 판단에 사용됩니다.
