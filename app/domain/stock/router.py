...
async def search_stock(
    ...
    mrkt_code: str = Query(None, description="시장 구분코드 (J:국내, NYS/NAS/AMS:미국 뉴욕/나스닥/아멕스)")
):
    """종목 검색"""
...
async def get_asking_price(
    ...
    mrkt_code: Annotated[str, Query(description="J:국내, NYS/NAS/AMS:미국")] = "J",
):
    """주식 호가 조회 (국내/미국)"""
    if is_overseas(mrkt_code):
        response = await foreign_api.get_inquire_asking_price(user_id, st_code, db, excd=mrkt_code)
    else:
...
async def fluctuation_rank(
    ...
    """등락률 순위 (국내/미국)"""
    if is_overseas(mrkt_code):
        response = await foreign_api.get_fluctuation_rank(user_id, db, rank_sort, excd=mrkt_code)
    else:
...
async def volume_rank(
    ...
    """거래량 순위 (국내/미국)"""
    if is_overseas(mrkt_code):
        response = await foreign_api.get_volume_rank(user_id, db, excd=mrkt_code)
    else:
...
async def volume_power_rank(
    ...
    """체결강도 순위 (국내/미국)"""
    if is_overseas(mrkt_code):
        response = await foreign_api.get_volume_power_rank(user_id, db, excd=mrkt_code)
    else:
