...
#   재시도로 늘리지 않고 대기를 늘리는 이유는, 재시도는 서버가 처음부터 다시
#   처리하게 만드는 반면 대기는 이미 처리 중인 응답을 그대로 받기 때문이다.
#   무엇보다 주문의 ReadTimeout 은 '체결됐는지 모르는 상태'를 만들고,
#   그 상태가 체결확인 실패 → 취소 실패 → 추적 불가로 이어진다.
#   응답을 받아내는 것이 그 연쇄의 입구를 막는 가장 싼 방법이다.
