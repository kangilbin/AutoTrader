...
# 스윙 생성
async def insert_swing_trade(db: AsyncSession, swing_data: SwingCreate):
    # 새로운 사용자 객체 생성
...
# 스윙 조회
async def select_swing_trade(db: AsyncSession, swing_id: int):
...
# 스윙 조회(계좌 번호)
async def select_swing_trade_account(db: AsyncSession, account_no: str):
