...
```
총 자본     = tot_evlu_amt(총평가금액 = 유가증권 평가금액 + D+2 예수금)
기존 할당   = SUM(SWING_TRADE.INIT_AMOUNT) WHERE ACCOUNT_NO = ? AND 시장 분류 일치 AND USE_YN = 'Y'
가용 자본   = 총 자본 - 기존 할당
```
...
WHERE ACCOUNT_NO = :account_no
  AND USE_YN = 'Y'  -- 활성 스윙만 할당 자본으로 집계
...
3. `output2`에서 총 자본 산출: `int(output2["tot_evlu_amt"])`
4. `repo.get_total_init_amount(account_no, overseas, exclude_swing_id)` 호출
5. `available_capital = total_capital - allocated` 계산
6. dict 반환

#### 3.3.2 `create_swing` — 자본 검증 불필요

등록 시 `USE_YN='N'`(비활성) 상태로 생성되므로 자본이 실제 할당되지 않는다.
자본 한도 검증은 `update_swing`에서 `USE_YN`이 `'Y'`로 활성화될 때 수행한다.
...
자본 한도 검증은 다음 두 경우에 수행한다:
1. **INIT_AMOUNT 변경** — 기존 할당 금액 변경 시
2. **USE_YN 활성화 (N→Y)** — 비활성 스윙이 활성화될 때 (create_swing 대신 여기서 검증)
...
async def update_swing(self, swing_id: int, data: dict, user_id: str = None) -> dict:
    try:
        ...
        # 1) INIT_AMOUNT 변경 시
        # 2) USE_YN 활성화 (N→Y) 시
        need_capital_check = False
        if "INIT_AMOUNT" in data and user_id:
            need_capital_check = True
        if data.get("USE_YN") == "Y" and swing.USE_YN == "N" and user_id:
            need_capital_check = True

        if need_capital_check:
            # 이미 활성 상태면 자기 자신 제외, 활성화 중이면 제외 불필요
            ...
            check_amount = data.get("INIT_AMOUNT", swing.INIT_AMOUNT)
...
| output2 필드 | 설명 | 용도 |
|-------------|------|------|
| `tot_evlu_amt` | 총평가금액 (유가증권 평가금액 + D+2 예수금) | **총 자본 산출에 사용** |
| `dnca_tot_amt` | 예수금총액 | 현금 자산 (참고용) |
| `scts_evlu_amt` | 주식평가금액 | 주식 자산 (참고용) |

**총 자본 계산**: `tot_evlu_amt`
...
- `mapping_swing`에서 자동 생성된 스윙 (매수 보유 종목 매핑)은 `INIT_AMOUNT=0`, `USE_YN='N'`
- `USE_YN='N'`이므로 `get_total_init_amount` 합계에서 자동 제외
